...
import { useUser } from '@clerk/clerk-react'; // Import depending on your auth provider

const AiTools = () => { 
    const navigate = useNavigate();
    const { user } = useUser(); // Make sure this is correctly imported
            ...
            <div className="flex flex-wrap mt-10 justify-center">
                {AiToolsData.map((tool, index) => (
                    ...
                    >
                        ...
                        <p className="text-gray-400 text-sm max-w-[95%]">{tool.subtitle}</p>
                    </div>
                ))}
            </div>
        </div>
